...
def write_json_to_file(data, file_path, logger=None):
...
def fetch(use_cache=False, logger=None):
...
def validate(tle_data, satellite_data, transmitter_data, logger=None):
...
def validate_norad_ids(data, logger):
...
def transform(tle_data, satellite_data, transmitter_data, logger=None):
...
def format(data, logger=None) -> dict:
    """Re-index the dictionary by satnogs 'sat_id' as the primary key"""
    logger("INFO", "Reindexing data to use sat_UUID as primary key.")
    satcom_db = {}
    for d in data:
        key = d["sat_id"]
        inner_dict = d.copy()
...
## Merge ##
def merge_with_je9pel(data: dict, je9pel_data: dict, logger=None):
    ...
    for sat_id, details in data.items():
        ...
        if norad_cat_id in je9pel_data:
            ...
            details["je9pel"] = None
            logger("INFO", f"NORAD CAT ID {norad_cat_id} in JE9PEL but not Satnogs DB. Skipping..")
...
def generate_db(use_cache=False, logger=None):
    if logger is None:
        logger = print
