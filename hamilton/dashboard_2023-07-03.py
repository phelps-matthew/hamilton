...
@app.callback(
    ...
    Output("tracking-interval-component", "max_intervals"),
...
def update_tracking_display(n, n_clicks, selected_rows, clickData, data):
    ...
    #        az = f"AZ: {az:<3.2f}"
    #        el = f"EL: {el:<3.2f}"
    #        #obs_params = so_tracker.calculate_observational_params(sat_id=selected_id)
    #        #az = f"AZ: {obs_params['az']:<3.2f}"
    #        #el = f"EL: {obs_params['el']:<3.2f}"
    #        return az, el
...
def track(sat_id):
    ...
    try:
        ...
        az_rot, el_rot = rot.status()
        az_rot, el_rot = rot.status()
        print(f"az_rot: {az_rot}")
        print(f"el_rot: {el_rot}")
    except Exception as e:
        print(e)
